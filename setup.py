...
Compiler notes:
    ...
    如果编译失败，可设置环境变量回退到纯 Python ctypes 模式：
        CHAN_C99_MODE=ctypes pip install .
"""
...
def _检测编译器_charset_flag():
    ...
    import warnings
    warnings.warn(
        "无法自动检测 C 编译器对 UTF-8 源码的支持。"
        "如果编译失败，请设置 CC 环境变量指向 GCC 或 Clang，"
        "或使用 CHAN_C99_MODE=ctypes 回退到纯 Python 模式。"
    )
    return base_args, link_args
