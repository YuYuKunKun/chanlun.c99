...
typedef struct K线合成器 K线合成器;
typedef struct 立体分析器 立体分析器;
...
struct 观察者 {
    ...
    动态数组 普通K线序列; /* K线* */
    动态数组 缠论K线序列; /* 缠论K线* */
    动态数组 *基础缠K序列; /* 缠论K线* — 自持或指向外部 */
    bool 基础缠K自持;           /* true = 自有 malloc 分配 */
...
#define K线合成器_最大周期数 8

typedef void (*K线合成器回调)(void *上下文, const char *信号类型,
                                    const char *标识, int 周期, K线 *完成K线);

struct K线合成器 {
    对象头;
    char 标识[64];
    int 周期数量;
    int 周期组[K线合成器_最大周期数];  /* 从小到大排序 */
    K线 *当前K线[K线合成器_最大周期数];  /* 正在合成中的K线, NULL = 无 */
    动态数组 合成K线列表[K线合成器_最大周期数];  /* 已完成的K线序列 */
    K线合成器回调 事件回调;
    void *回调上下文;
};
