...
static int 相对方向_从名称(const char *s, 相对方向 *out) {
    if (strcmp(s, "向上") == 0) { *out = 相对方向_向上; return 1; }
    if (strcmp(s, "向下") == 0) { *out = 相对方向_向下; return 1; }
    if (strcmp(s, "向上缺口") == 0) { *out = 相对方向_向上缺口; return 1; }
    if (strcmp(s, "向下缺口") == 0) { *out = 相对方向_向下缺口; return 1; }
    if (strcmp(s, "衔接向上") == 0) { *out = 相对方向_衔接向上; return 1; }
    if (strcmp(s, "衔接向下") == 0) { *out = 相对方向_衔接向下; return 1; }
    if (strcmp(s, "顺") == 0) { *out = 相对方向_顺; return 1; }
    if (strcmp(s, "逆") == 0) { *out = 相对方向_逆; return 1; }
    if (strcmp(s, "同") == 0) { *out = 相对方向_同; return 1; }
    return 0;
}

static int 分型结构_从名称(const char *s, 分型结构 *out) {
    if (strcmp(s, "顶") == 0) { *out = 分型结构_顶; return 1; }
    if (strcmp(s, "底") == 0) { *out = 分型结构_底; return 1; }
    if (strcmp(s, "上") == 0) { *out = 分型结构_上; return 1; }
    if (strcmp(s, "下") == 0) { *out = 分型结构_下; return 1; }
    if (strcmp(s, "散") == 0) { *out = 分型结构_散; return 1; }
    return 0;
}
...
typedef struct { ChanObject base; } ChanKLineObject;
...
typedef struct { ChanObject base; } GapObject;
...
typedef struct { ChanObject base; } FractalObject;
...
typedef struct { ChanObject base; } DashLineObject;
...
typedef struct { ChanObject base; } StrokeObject;
...
typedef struct { ChanObject base; } SegFeatureObject;
...
static int Py_线段特征_设置_序号(ChanObject *self, PyObject *value, void *c) {
    long v = PyLong_AsLong(value); if (v==-1 && PyErr_Occurred()) return -1;
    ((线段特征*)self->ptr)->序号 = (int)v; return 0;
}
...
typedef struct { ChanObject base; } SegmentObject;
...
typedef struct { ChanObject base; } HubObject;
...
static PyObject *Py_动态数组_new(PyTypeObject *type, PyObject *args, PyObject *kw) {
    ...
    if (!self) return NULL;
    动态数组_初始化(&self->arr, 4);
    self->owns_elements = true;  /* user-created: append() calls 引用() */
...
/* append(py_元素) — transfers ownership via 引用() */
...
static PyMethodDef DynArray_methods[] = {
    ...
    {"clear",     (PyCFunction)Py_动态数组_清空,     METH_NOARGS,
     "Clear all elements (calls 解引用 on each)."},
    {NULL}
};
    ...
    int mode)  /* 0=slope, 1=measure, 2=macd, 3=full, 4=any, 5=config, 6=optional, 7=模式字符串 */
{
    ...
    bool py_结果 = false;
    switch (mode) {
        case 0: py_结果 = 背驰分析_斜率背驰(enter, leave); break;
        case 1: py_结果 = 背驰分析_测度背驰(enter, leave); break;
        case 2:
