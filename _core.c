...
/* Forward declarations for 缠论K线 classmethods (matching chan.py 缠论K线) */
...
/* Forward declarations for 分型 methods (matching chan.py 分型) */
...
static PyObject *Py_制作_相对方向(int val) {
    if (!相对方向_类) return PyUnicode_FromString(相对方向_到名称((相对方向)val));
...
static PyObject *Py_缠论K线_获取_与RSI匹配(ChanObject *self, void *c) {
    缠论K线 *ck = (缠论K线 *)self->ptr;
    if (!ck->标的K线 || !ck->标的K线->rsi) Py_RETURN_FALSE;
    double rsi = ck->标的K线->rsi->RSI;
    double sma = ck->标的K线->rsi->RSI_SMA;
    if (ck->分型 == 分型结构_底 || ck->分型 == 分型结构_下)
        return PyBool_FromLong(rsi < sma);
    if (ck->分型 == 分型结构_顶 || ck->分型 == 分型结构_上)
        return PyBool_FromLong(rsi > sma);
    Py_RETURN_FALSE;
}
...
static PyObject *Py_缠论K线_获取_与KDJ匹配(ChanObject *self, void *c) {
    缠论K线 *ck = (缠论K线 *)self->ptr;
    if (!ck->标的K线 || !ck->标的K线->kdj) Py_RETURN_FALSE;
    if (ck->标的K线->kdj->K < 0 || ck->标的K线->kdj->D < 0) Py_RETURN_FALSE;
    double kval = ck->标的K线->kdj->K;
    double dval = ck->标的K线->kdj->D;
    if (ck->分型 == 分型结构_底 || ck->分型 == 分型结构_下)
        return PyBool_FromLong(kval < dval);
    if (ck->分型 == 分型结构_顶 || ck->分型 == 分型结构_上)
        return PyBool_FromLong(kval > dval);
    Py_RETURN_FALSE;
}
...
typedef struct { ChanObject base; } MACDObject;
...
static PyObject *Py_MACD_获取_快线周期(ChanObject *self, void *c) {
    (void)c; return PyLong_FromLong(((平滑异同移动平均线 *)self->ptr)->快线周期);
}
static PyObject *Py_MACD_获取_慢线周期(ChanObject *self, void *c) {
    (void)c; return PyLong_FromLong(((平滑异同移动平均线 *)self->ptr)->慢线周期);
}
static PyObject *Py_MACD_获取_信号周期(ChanObject *self, void *c) {
    (void)c; return PyLong_FromLong(((平滑异同移动平均线 *)self->ptr)->信号周期);
}
...
typedef struct { ChanObject base; } RSIObject;

static PyObject *Py_RSI_获取_RSI(ChanObject *self, void *c) {
    (void)c; return PyFloat_FromDouble(((相对强弱指数 *)self->ptr)->RSI);
}
static PyObject *Py_RSI_获取_RSI_SMA(ChanObject *self, void *c) {
    (void)c; return PyFloat_FromDouble(((相对强弱指数 *)self->ptr)->RSI_SMA);
}
static PyObject *Py_RSI_获取_周期(ChanObject *self, void *c) {
    (void)c; return PyLong_FromLong(((相对强弱指数 *)self->ptr)->周期);
}
...
typedef struct { ChanObject base; } KDJObject;

static PyObject *Py_KDJ_获取_K(ChanObject *self, void *c) {
    (void)c; return PyFloat_FromDouble(((随机指标 *)self->ptr)->K);
}
static PyObject *Py_KDJ_获取_D(ChanObject *self, void *c) {
    (void)c; return PyFloat_FromDouble(((随机指标 *)self->ptr)->D);
}
static PyObject *Py_KDJ_获取_J(ChanObject *self, void *c) {
    (void)c; return PyFloat_FromDouble(((随机指标 *)self->ptr)->J);
}
...
static PyObject *Py_分型_获取_强度(ChanObject *self, void *c) {
    (void)c;
    分型 *f = (分型 *) self->ptr;
    if (f->结构 != 分型结构_顶 && f->结构 != 分型结构_底)
        return PyUnicode_FromString("未知");
    if (!f->右 || !f->左)
        return PyUnicode_FromString("未知");
    if (f->结构 == 分型结构_底) {
        if (f->右->标的K线->收盘价 > f->左->标的K线->高)
            return PyUnicode_FromString("强");
        else if (f->右->标的K线->收盘价 > f->中->标的K线->高)
            return PyUnicode_FromString("中");
        else
            return PyUnicode_FromString("弱");
    } else {
        if (f->右->标的K线->收盘价 < f->左->标的K线->低)
            return PyUnicode_FromString("强");
        else if (f->右->标的K线->收盘价 < f->中->标的K线->低)
            return PyUnicode_FromString("中");
        else
            return PyUnicode_FromString("弱");
    }
...
static PyObject *Py_分型_获取_与MACD柱子分型匹配(ChanObject *self, void *c) {
    (void)c;
    分型 *f = (分型 *) self->ptr;
    if (!f->右 || !f->左) Py_RETURN_FALSE;
    if (f->结构 == 分型结构_底) {
        if (f->左->标的K线->macd->MACD柱 > f->中->标的K线->macd->MACD柱 &&
            f->中->标的K线->macd->MACD柱 < f->右->标的K线->macd->MACD柱)
            Py_RETURN_TRUE;
    } else if (f->结构 == 分型结构_顶) {
        if (f->左->标的K线->macd->MACD柱 < f->中->标的K线->macd->MACD柱 &&
            f->中->标的K线->macd->MACD柱 > f->右->标的K线->macd->MACD柱)
...
static PyObject *Py_中枢_repr(ChanObject *self) {
    ...
    char 缓冲[384];
    if (h->基础序列.长度 > 0) {
        虚线 *首 = (虚线 *) 动态数组_获取(&h->基础序列, 0);
        虚线 *末 = (虚线 *) 动态数组_获取(&h->基础序列, h->基础序列.长度 - 1);
        snprintf(缓冲, sizeof(缓冲),
...
static PyObject *Py_K线_截取(PyObject *cls, PyObject *args) {
    ...
    if (始_idx < 0) return NULL;
    Py_ssize_t 终_idx = PySequence_Index(序列, 终);
    if (终_idx < 0) return NULL;
...
static PyObject *Py_缠论K线_创建缠K(PyObject *cls, PyObject *args, PyObject *kw) {
    ...
    相对方向 方向;
    if (!解析方向(py_方向, &方向)) return NULL;

    分型结构 结构;
    if (!解析分型结构(py_结构, &结构)) return NULL;
...
static PyObject *Py_序列视图_下标(ObserverSeqView *self, PyObject *key) {
    ...
    if (PySlice_Check(key)) {
        ...
        if (start < 0) start = 0;
        if (stop > n) stop = n;
        if (start >= stop) return PyList_New(0);
        PyObject *list = PyList_New(stop - start);
        if (!list) return NULL;
        for (Py_ssize_t k = start; k < stop; k++) {
            void *ptr = self->at_fn((观察者 *) self->obs->base.ptr, (size_t) k);
            if (!ptr) {
                Py_DECREF(list);
                PyErr_Format(PyExc_IndexError, "%s[%zd] is NULL", self->name, k);
                return NULL;
            }
            PyList_SET_ITEM(list, k - start, Py_制作_借用(self->item_type, ptr));
        }
        return list;
    }
    PyErr_SetString(PyExc_TypeError, "sequence index must be integer or slice");
    return NULL;
}
...
OBS_SEQ_GETTER(strokes, strokes, &DashLine_Type)
OBS_SEQ_GETTER(stroke_hubs, stroke_hubs, &Hub_Type)
OBS_SEQ_GETTER(segments, segments, &DashLine_Type)
OBS_SEQ_GETTER(hubs, hubs, &Hub_Type)
OBS_SEQ_GETTER(ext_segments, ext_segments, &DashLine_Type)
OBS_SEQ_GETTER(ext_hubs, ext_hubs, &Hub_Type)
OBS_SEQ_GETTER(ext_segments_seg, ext_segments_seg, &DashLine_Type)
OBS_SEQ_GETTER(ext_hubs_seg, ext_hubs_seg, &Hub_Type)
OBS_SEQ_GETTER(seg_seg, seg_seg, &DashLine_Type)
OBS_SEQ_GETTER(seg_hubs, seg_hubs, &Hub_Type)
OBS_SEQ_GETTER(ext_seg_extseg, ext_seg_extseg, &DashLine_Type)
...
PyMODINIT_FUNC PyInit__core(void) {
    ...
    if (PyType_Ready(&MACD_Type) < 0) return NULL;
    if (PyType_Ready(&RSI_Type) < 0) return NULL;
    if (PyType_Ready(&KDJ_Type) < 0) return NULL;
