...
void 释放全局内存池(void) {
    ...
    while (cursor) {
        if (!已销毁(cursor)) {
            int wrc = 弱引用计数_取(cursor);
            if (wrc != 0) {
                fprintf(stderr, "WEAKREF_WARN: type=%d(%s) ptr=%p 弱引用计数=%d\n",
                    (int)对象类型_取(cursor), 类型名称[(int)对象类型_取(cursor)], cursor, wrc);
                /* 修复弱引用计数到 0 以便继续 */
                ((对象头结构*)cursor)->弱引用计数 = 0;
            }
            对象销毁(cursor);
        }
...
void 动态数组_初始化(动态数组* arr, size_t 初始容量) {
    ...
    if (!arr->数据) { perror("动态数组_初始化"); chan_oom_handler(初始容量 * sizeof(void*)); return; }
    arr->长度 = 0;
    arr->容量 = 初始容量;
}
...
void 动态数组_追加(动态数组* arr, void* 元素) {
    if (arr->长度 >= arr->容量) {
        if (arr->容量 > SIZE_MAX / 2) { perror("动态数组_追加"); chan_oom_handler(SIZE_MAX); return; }
        size_t 新容量 = arr->容量 * 2;
        void** 新数据 = realloc(arr->数据, 新容量 * sizeof(void*));
        if (!新数据) { perror("动态数组_追加"); chan_oom_handler(新容量 * sizeof(void*)); return; }
        arr->数据 = 新数据;
        arr->容量 = 新容量;
    }
    arr->数据[arr->长度++] = 元素;
...
#define 弱引用_设置(ptr, field, target) do { \
    if ((ptr)->field) ((对象头结构*)((ptr)->field))->弱引用计数--; \
    (ptr)->field = (target); \
    if (target) ((对象头结构*)(target))->弱引用计数++; \
} while(0)
...
void 弱引用_数组清除(动态数组* arr) {
    ...
    free(arr->数据);
    arr->数据 = NULL; arr->长度 = 0; arr->容量 = 0;
}

/* 旧名称兼容（逐步废弃） */
#define 虚线指针列表_追加(列表, 元素)         弱引用_数组追加(列表, 元素)
#define 虚线指针列表_移除末位(列表)           弱引用_数组弹出(列表)
#define 虚线指针列表_全部解引用(列表)         弱引用_数组清除(列表)
#define 线段特征指针列表_全部解引用(列表)     弱引用_数组清除(列表)
#define 中枢指针列表_全部解引用(列表)         弱引用_数组清除(列表)
    ...
    for (size_t idx = 0; idx < 虚线序列->长度; idx++) {
        ...
        if (虚线_方向(当前虚线) == 线段方向) {
            ...
            if ((线段方向 == 相对方向_向上 && 结构 == 分型结构_顶 && 虚线_高(当前虚线) > 线段特征_高(中)) ||
                ...
                if (!tmp2.数据) { perror("线段特征_静态分析"); chan_oom_handler(sizeof(void*)); return; }
                动态数组_追加(&tmp2, fake);
                线段特征* new_ft = 线段特征_新建("特征", &tmp2, 线段方向);                free(tmp2.数据);
                弱引用_数组弹出(结果);
        ...
        if (结果->长度 == 0) {
            ...
            if (!tmp.数据) { perror("线段特征_静态分析"); chan_oom_handler(sizeof(void*)); return; }
            动态数组_追加(&tmp, 当前虚线);
    ...
    if (序列[2]) {
        ...
        if (new_base.长度 >= 6 && new_base.长度 % 2 == 0) {
            for (size_t i_ = 0; i_ < new_base.长度; i_++)
                弱引用计数_取(动态数组_获取(&new_base, i_))++;
            for (size_t i_ = 0; i_ < 段->基础序列.长度; i_++)
                弱引用计数_取(动态数组_获取(&段->基础序列, i_))--;
...
void 线段_刷新(虚线* 段, 缠论配置* 配置) {
    ...
    if (!eff) { perror("线段_武斗"); chan_oom_handler(3 * sizeof(线段特征*)); return; }
    int effn = 0;
    for (int i = 0; i < 3; i++) {
...
void 线段_序列重置(虚线* 段, 动态数组* 序列) {
    ...
    for (size_t i = 0; i < new_base.长度; i++)
        弱引用计数_取(动态数组_获取(&new_base, i))++;
    for (size_t i = 0; i < 段->基础序列.长度; i++)
        弱引用计数_取(动态数组_获取(&段->基础序列, i))--;
    动态数组_释放(&段->基础序列, false);
    段->基础序列 = new_base;
    弱引用_数组设置(&段->特征序列, 2, NULL);
}
...
static void 线段_添加线段(动态数组* 线段序列, 虚线* 待添加, 缠论配置* 配置) {
    ...
    if (线段序列->长度 == 0) { 弱引用_数组追加(线段序列, 待添加); return; }
...
static bool 线段_非缺口下穿刺(动态数组* 线段序列, 缠论配置* 配置) {
    ...
    if (动态数组_获取(&cur2->特征序列, 2)) {
        ...
        动态数组 tmp = { .数据 = malloc(sizeof(void*)), .长度 = 0, .容量 = 1 };
        if (!tmp.数据) { perror("线段_非缺口下穿刺"); chan_oom_handler(sizeof(void*)); return false; }
...
static bool 线段_修正(动态数组* 线段序列, 缠论配置* 配置) {
    ...
    if (strcmp(线段_四象(cur), "老阴") == 0 || strcmp(线段_四象(cur), "老阳") == 0) {
        弱引用_设置(seg1, 前一缺口, NULL); seg1->前一缺口 = NULL;
    }
    ...
    while (深度 < 512) {
        ...
        if (线段序列->长度 == 0) {
            for (size_t i = 1; i + 1 < 笔序列->长度; i++) {
                ...
                /* seg->特征序列 already initialized with 3 NULLs by 虚线_创建线段 */
                动态数组 tmp = { .数据 = malloc(sizeof(void*)), .长度 = 0, .容量 = 1 };
                if (!tmp.数据) { perror("线段_扩展分析"); chan_oom_handler(sizeof(void*)); return; }
                动态数组_追加(&tmp, 中);
                线段特征* ft = 线段特征_新建("特征", &tmp, 虚线_方向(seg));
                free(tmp.数据);
                弱引用_数组设置(&seg->特征序列, 0, ft);
        ...
        if (动态数组_获取(&cur->特征序列, 2) != NULL) {
            ...
            if (strcmp(sx_cur, "老阴") == 0 || strcmp(sx_cur, "老阳") == 0) {
                弱引用_设置(seg, 前一缺口, NULL); seg->前一缺口 = NULL;
            }
            动态数组_释放(&前, false); 动态数组_释放(&后, false); 动态数组_释放(&第三, false);
        }
...
void 线段_验证序列(虚线* 段, 动态数组* 序列) {
    ...
    if (段->基础序列.长度 % 2 == 0 && 段->基础序列.长度 > 0) {
        弱引用计数_取(动态数组_获取(&段->基础序列, 段->基础序列.长度 - 1))--;
        段->基础序列.长度--;
    }
}
...
void 读取数据文件(const char* 文件路径) {
    ...
    /* 清理：释放所有动态数组内部缓冲区（false = 不解引用元素，池拥有元素内存） */
    动态数组_释放(&普通K线序列, false);
    动态数组_释放(&缠论K线序列, false);
    动态数组_释放(&分型序列, false);
    动态数组_释放(&笔序列, false);
    动态数组_释放(&线段序列, false);
    动态数组_释放(&中枢序列, false);
