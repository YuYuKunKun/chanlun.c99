...
/* 手动弱引用操作（用于手动平衡场景，含追踪） */
#define 弱引用_手动增加(ptr) do { \
    ((对象头结构*)(ptr))->弱引用计数++; \
    _追踪_操作("手动增加", ptr, +1); \
} while(0)
...
/* 弱引用追踪 — 调试用，排查 phantom refs */
static bool _追踪_启用 = false; /* 调试用 */
static bool _追踪_详细 = false; /* 调试用 */
static void *_追踪_目标 = NULL;
...
static int _追踪_统计数 = 0;

static int _追踪_查找或创建(const char *op) {
    for (int i = 0; i < _追踪_统计数; i++)
        if (strcmp(_追踪_统计[i].name, op) == 0) return i;
    int i = _追踪_统计数++;
    _追踪_统计[i].name = op;
    _追踪_统计[i].inc = 0;
    _追踪_统计[i].dec = 0;
...
void _追踪_打印统计(void) {
    ...
    int net = 0;
    for (int i = 0; i < _追踪_统计数; i++) {
        int _n = _追踪_统计[i].inc - _追踪_统计[i].dec;
        fprintf(stderr, "%-20s %8d %8d %+8d\n",
                _追踪_统计[i].name, _追踪_统计[i].inc, _追踪_统计[i].dec, _n);
        net += _n;
    }
    fprintf(stderr, "%-20s %8s %8s %+8d\n", "合计", "", "", net);
}

static void _追踪_操作_数组(const char *op, void *target, int delta, void *arr) {
    if (!_追踪_启用 || !target) return;
    if (对象类型_取(target) != CHAN_TYPE_虚线) return;
    int idx = _追踪_查找或创建(op);
    if (delta > 0) _追踪_统计[idx].inc++;
    else _追踪_统计[idx].dec++;
    if (_追踪_详细) {
        虚线 *d = (虚线 *) target;
        int after = ((对象头结构 *) target)->弱引用计数;
        fprintf(stderr, "[WEAK_TRACE] %s ptr=%p arr=%p 标识=%s 序号=%d delta=%+d after=%d\n",
                op, target, arr, d->标识, d->序号, delta, after);
    }
}

static void _追踪_操作(const char *op, void *target, int delta) {
    _追踪_操作_数组(op, target, delta, NULL);
}
...
void 弱引用_数组设置(动态数组 *arr, size_t i, void *新元素) {
    ...
    if (旧) {
        ((对象头结构 *) 旧)->弱引用计数--;
        _追踪_操作_数组("数组设置_旧", 旧, -1, arr);
    }
    动态数组_设置(arr, i, 新元素);
    if (新元素) {
        ((对象头结构 *) 新元素)->弱引用计数++;
        _追踪_操作_数组("数组设置_新", 新元素, +1, arr);
    }
}
