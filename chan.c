...
void 弱引用_数组追加(动态数组 *arr, void *基础序列) {
    动态数组_追加(arr, 基础序列);
    if (基础序列) __atomic_fetch_add(&((对象头结构 *) 基础序列)->弱引用计数, 1, __ATOMIC_RELAXED);
}
        ...
        if (虚线_方向(当前虚线) == 线段方向) {
            ...
            if ((线段方向 == 相对方向_向上 && 结构 == 分型结构_顶 && 虚线_高(当前虚线) > 线段特征_高(中)) ||
                ...
                for (size_t j = 1; j < 中->基础序列.长度; j++) {
                    if (((虚线 *) 中->基础序列.数据[j])->序号 < 小号->序号) 小号 = 中->基础序列.数据[j];
                }
                for (size_t j = 1; j < 右->基础序列.长度; j++) {
                    if (((虚线 *) 右->基础序列.数据[j])->序号 > 大号->序号) 大号 = 右->基础序列.数据[j];
                }
...
double 中枢_高高(中枢 *self) {
    double h = -INFINITY;
    size_t n = self->基础序列.长度 > 3 ? self->基础序列.长度 : 3;
...
bool 中枢_校验合法性(中枢 *self, 动态数组 *序列, 动态数组 *中枢序列) {
    ...
    /* 截断无效元素并递减弱引用计数 */
    while (self->基础序列.长度 > 有效长度) 弱引用_数组弹出(&self->基础序列);
    ...
    while (self->基础序列.长度 > 有效长度) 弱引用_数组弹出(&self->基础序列);
    if (self->基础序列.长度 < 3) return false;
    ...
    if (!相对方向_是否缺口(相对方向_分析(
        ((虚线 *) self->基础序列.数据[0])->高, ((虚线 *) self->基础序列.数据[0])->低,
        ((虚线 *) self->基础序列.数据[2])->高, ((虚线 *) self->基础序列.数据[2])->低))) {
...
const char *中枢_当前状态(中枢 *self) {
    分型 *尾部 = ((虚线 *) 动态数组_获取(&self->基础序列, self->基础序列.长度 - 1))->武;
    虚线 *last = 动态数组_获取(&self->基础序列, self->基础序列.长度 - 1);
...
void 观察者_重置基础序列(观察者 *self) {
    弱引用_数组清除(&self->普通K线序列);
    弱引用_数组清除(&self->缠论K线序列);
    弱引用_数组清除(&self->基础缠K序列);
    弱引用_数组清除(&self->分型序列);
    弱引用_数组清除(&self->笔序列);
    弱引用_数组清除(&self->笔_中枢序列);
    弱引用_数组清除(&self->线段序列);
    弱引用_数组清除(&self->中枢序列);
    弱引用_数组清除(&self->扩展线段序列);
    弱引用_数组清除(&self->扩展中枢序列);
    弱引用_数组清除(&self->扩展线段序列_线段);
    弱引用_数组清除(&self->扩展中枢序列_线段);
    弱引用_数组清除(&self->线段_线段序列);
    弱引用_数组清除(&self->线段_中枢序列);
    弱引用_数组清除(&self->扩展线段序列_扩展线段);
    弱引用_数组清除(&self->扩展中枢序列_扩展线段);
}
...
int main(int argc, char **argv) {
    ...
    for (size_t i = 0; i < obs->中枢序列.长度; i++) {
        ...
        fprintf(f枢, "%d %d %s %.6f %.6f %zu",
                z->序号, z->级别, z->标识,
                中枢_高(z), 中枢_低(z), z->基础序列.长度);
        for (size_t j = 0; j < z->基础序列.长度; j++) {
            虚线 *e = 动态数组_获取(&z->基础序列, j);
